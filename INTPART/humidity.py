...
plt.plot(totaltemps,linewidth=2,color='darkorange',zorder=2)   
plt.plot([0]*len(totaltemps),linewidth=3,color='k',linestyle='--',zorder=1)  

plt.xlim([0,len(totaltemps)])
plt.ylim([-6,6])

fig.suptitle(r'\textbf{SST (}$^{\circ}$\textbf{C)}',fontsize=13)   
        
plt.savefig(directoryfigure + 'SST.png',dpi=300)                     